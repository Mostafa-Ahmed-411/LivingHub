  ...
  try {
    ...
    if (isEmail) {
      await sendEmail({
        ...
        message: `Welcome to LivingHub, ${fullName}!\n\Your verification code is: ${verificationCode}\nThis code will expire in 15 minutes.`
      });
    } else {
      if (process.env.NODE_ENV === 'production') {
        // SMS Gateway not implemented in MVP, so we explicitly fail instead of silently succeeding
        throw new AppError('SMS service is not configured in production. Please use an email address to sign up.', 501);
      } else {
...
const login = async (req, res, next) => {
  try {
    ...
    if (user.accountStatus === 'rejected') {
      throw new AppError('Your account has been rejected. Contact support for assistance.', 403);
    }
...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    res.json({ message: 'If an account exists with this email/phone, a reset link has been sent.' });
  } catch (error) {
    next(error);
  }
};
